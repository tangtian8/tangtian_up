...
To retrieve a list of `pojo.User` objects in Java, you would normally write the following:

[source,java,indent=0]
----
	Flux<pojo.User> users  = client.get().retrieve().bodyToFlux(pojo.User.class)
----
...
----
	val users = client.get().retrieve().bodyToFlux<pojo.User>()
	// or (both are equivalent)
	val users : Flux<pojo.User> = client.get().retrieve().bodyToFlux()
----
...
class UserHandler(builder: WebClient.Builder) {
	...
	suspend fun listView(request: ServerRequest): ServerResponse =
			...
			client.get().uri("...").awaitExchange().awaitBody<pojo.User>()))
...
* `equals()` and `hashCode()`
* `toString()` of the form `"pojo.User(name=John, age=42)"`
